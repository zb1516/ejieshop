...
class Login extends BaseService
{
    ...
    public function loginMpWx(array $form): bool
    {
        ...
        //推荐人ID
        $refereeId = $form['refereeId'] ?? 0;
        if(!empty($refereeId)){
            $reuserInfo = UserModel::detail($refereeId);
            if(empty($reuserInfo)){
                throwError('推荐人信息不存在', null);
            }    
        }                 
        // 用户信息存在, 更新登录信息
        if (!empty($userInfo)) {
        ...
        if (!$setting['isForceBindMpweixin']) {
            // 用户不存在: 创建一个新用户
            $this->createUser('', true, $refereeId, $form['partyData']);
            // 保存第三方用户信息
    ...
    private function register(array $data): void
    {
        ...
        // 用户不存在: 创建一个新用户
        $this->createUser($data['mobile'], $data['isParty'], $refereeId, $data['partyData']);
    }
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    private function createUser(string $mobile, bool $isParty, int $refereeId = 0, array $partyData = []): void
    {
        // 用户信息
        $data = [
            'mobile' => $mobile,
