...
class Checkout extends BaseService
{
     ...
     * @param int $cardLogId 会员权益领取记录表ID
     * @return bool
     */
    public function createOrder(array $order, int $cardLogId = 0): bool
    {
        // 表单验证
        ...
        if ($status && $order['payType'] == OrderPayTypeEnum::BALANCE) {
            return $this->model->onPaymentByBalance($this->model['order_no'], $cardLogId);
        }
    ...
    private function add($order, string $remark = ''): bool
    {
        ...
        //微信支付才进行分润
        if($order['payType'] == OrderPayTypeEnum::WECHAT){
            //获取分润金额与邀请人
            $refree = UserService::getMaxCommission($order['orderPayPrice']);
        }
        // 订单数据
        $data = [
            'user_id' => $this->user['user_id'],
            ...
            'refree_price' => $refree['refree_price'] ?? 0,
            'refereeId' => $refree['refereeId'] ?? 0,
        ];
        if ($order['delivery'] == DeliveryTypeEnum::EXPRESS) {
     ...
     * @return void
     */
    private function userCommission(array $order)
    {
        $userInfo = UserModel::detail((int) $this->user['user_id']);
        //通过推荐码查询推荐人
        $user = UserModel::detail(['referee_id' => $userInfo['referee_id']]);
        //获取推荐人设置的返佣比例
        $commission = CommissionModel::detail(['scale_name' => 'referee']);
        $scaleValue = empty($commission['scale_value']) ? 0 : $commission['scale_value'];
        $price = bcsub($order['order_price'], $order['coupon_money']); //减去优惠券抵扣金额
        $payPrice = bcsub($price, $order['points_money']); //减去积分抵扣金额
        $refereePrice = bcmul($payPrice, $scaleValue); //计算返佣金额
        return $refereePrice;
    }
}
