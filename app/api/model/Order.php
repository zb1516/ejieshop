...
class Order extends OrderModel
{
     ...
     * @return bool
     */
    public function onPaymentByBalance(string $orderNo, int $cardLogId = 0): bool
    {
        // 获取订单详情
        ...
        //如果ccardLogId存在直接更新一下会员卡权益领取状态
        if($cardLogId > 0){
            (new MembeShipCardLog())->where('id', $cardLogId)->update(['update_at' => time(), 'status' => 1]);
        }
        return $status;
    }
    ...
    public function receipt()
    {
        ...
        return $this->transaction(function () {
            ...
            //添加分润记录
            if($this['refereeId'] > 0 && $this['refree_price'] > 0){
                //累积用户可用余额
                User::setIncBalance((int) $this['refereeId'], (float) $this['refree_price']);
                // 新增余额变动记录
                BalanceLog::add(SceneEnum::RETURNPRICE , [
                    'user_id' => $this['refereeId'],
                    'money' => +$this['refree_price'],
    ...
    {
       return $this->where($where)->where('is_delete','=',0)->select();
    }
}
