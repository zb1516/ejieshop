...
class Goods extends GoodsModel
{
    ...
    {
        // 整理查询参数
        $params = array_merge($param, ['status' => GoodsStatusEnum::ON_SALE, 'type' => 1, 'business_id' => 1]); //1普通商品2:0元购商品
        // 获取商品列表只展示普通商品
    ...
    public function getDetails(int $goodsId, bool $verifyStatus = true)
    {
        ...
        foreach($goodsInfo['skuList'] as &$skugoods){
            $skugoods['discount_price'] = bcmul((string) $skugoods['goods_price'], User::getMinDiscount(), 2);
        }
        $goodsInfo['discount_price'] = bcmul((string) $goodsInfo['goods_price_min'], User::getMinDiscount(), 2);
