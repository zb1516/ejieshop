...
class Order extends OrderModel
{
     ...
     */
    public function createOrder(int $planId = 0, int $cardId = 0, float $customMoney = 0.00)
    {
        // 确定充值方式
        $rechargeType = RechargeTypeEnum::CUSTOM;
        if ($planId > 0) {
            $rechargeType = RechargeTypeEnum::PLAN;
        }
        if ($cardId > 0) {
            $rechargeType = RechargeTypeEnum::CARD;
        }
        // 验证用户输入
        if (!$this->validateForm($rechargeType, $planId, $cardId, $customMoney)) {
            $this->error = $this->error ?: '数据验证错误';
            return false;
        }
        // 获取订单数据
        $data = $this->getOrderData($rechargeType, $planId, $cardId, $customMoney);
        // 记录订单信息
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    private function getOrderData(int $rechargeType, int $planId, int $cardId, float $customMoney)
    {
        // 订单信息
        $data = [
            'order' => [
        ...
        if ($rechargeType == RechargeTypeEnum::CARD) {
            $data = $this->createDataByCard($data, $cardId);
        }
        // 实际到账金额
     ...
     * @throws BaseException
     */
    private function createDataByCard(array $order, int $cardId)
    {
        // 获取套餐详情
        $cardInfo = MembeShipCardConfig::detail($cardId);
        if (empty($cardInfo)) {
            throwError('会员卡不存在');
        }
        $order['plan'] = [
            'plan_id' => 0,
            'card_id' => $cardId,
            'plan_name' => $cardInfo['name'],
            'money' => $cardInfo['amount'],

        ];
        $order['order']['card_id'] = $cardInfo['id'];
        $order['order']['gift_money'] = 0;
        $order['order']['pay_price'] = $cardInfo['amount'];
        return $order;
    }
    /**
     * 创建套餐充值订单数据
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    private function validateForm(int $rechargeType, int $planId, int $cardId, float $customMoney)
    {
        if ($rechargeType == RechargeTypeEnum::PLAN) {
        ...
        if ($rechargeType == RechargeTypeEnum::CARD) {
            if (empty($cardId) && $customMoney <= 0) {
                $this->error = '请选择充值会员卡或输入充值金额';
                return false;
            }
        }
        // 验证自定义的金额
