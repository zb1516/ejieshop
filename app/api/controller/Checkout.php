...
class Checkout extends Controller
{
    ...
    private function buyNow(): Json
    {
        ...
        $cardLogId = (int) $params['cardLogId'] ?? 0;
        if($orderInfo['goodsList'][0]['type'] == 2){
            if($cardLogId == 0){
                throwError('缺少参数card_log_id');
            }
            //0元购商品一次只能购买一个
            if($params['goodsNum'] > 1){
                throwError('0元购商品一次只能购买一个');
            } 
            //是不是当前用户领取的权益或赠品
            $user_id = UserService::getCurrentLoginUserId(true);
            $data = [
                ['user_id', '=', $user_id],
                ['id', '=', $cardLogId],
                ['status', '=', 0],
            ...
            $cardlog = MembeShipCardLog::detail($data);
            if(empty($cardlog)){
                throwError('无法领取权益商品及赠品');
            }
        }
        // 创建订单
        if (!$Checkout->createOrder($orderInfo, $cardLogId)) {
