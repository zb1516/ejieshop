...
class Order extends Controller
{
     ...
     * @throws BaseException
     */
    function list(string $dataType): Json {
        $model = new OrderModel;
     ...
     * @return void
     */
    private function refreePriceOrder(int $userId, int $orderId)
    {
        $userInfo = \app\common\model\User::detail(['user_id' => $userId]);
        if (!empty($userInfo['refereeId'])) {
            $userInfo = \app\common\model\User::detail(['user_id' => $userInfo['refereeId']]);
            //通过订单id查询返佣金额
            $orderGoodsModel = new OrderGoodsModel;
            $orderInfo = $orderGoodsModel->detail(['order_id' => $orderId]);
            //手动确认收货给推荐人返佣
            UserModel::setIncBalance((int) $userInfo['user_id'], (float) $orderInfo['refree_price']);
            // 新增余额变动记录
            BalanceLogModel::add(SceneEnum::RETURNPRICE , [
                'user_id' => (int) $userInfo['user_id'],
                'money' => +$orderInfo['refree_price'],
