...
class Goods extends BaseModel
{
    ...
    // 追加字段
    protected $append = ['goods_sales', 'title'];
    /**
     * 获取器：门店标题
     * @param $value
     * @param $data
    ...
    {
        return Business::where(['id' => $data['business_id']])->value('title');
    }
    /**
     * 关联模型：主图视频文件
     * @return HasOne
    ...
    private function getQueryFilter(array $param)
    {
        ...
        //商品类型 1:普通商品 2:0元购商品前台只展示普通商品
        $params['type'] > 0 && $filter[] = ['type', '=', (int)$params['type']];
        // 门店Id
        $params['business_id'] > 0 && $filter[] = ['business_id', '=', (int)$params['business_id']];
        // 商品分类
    ...
    protected function setGoodsData($goodsInfo, callable $callback = null)
    {
        ...
        // 商品主图
        $goodsInfo['goods_image'] = current($goodsInfo['goods_images']) == false?'': current($goodsInfo['goods_images'])['preview_url'];
     ...
     * @return array|mixed
     */
    public function getListByBusinessId(int $storeId, $status = null)
    {
        // 筛选条件
        $filter = [['business_id', '=', $storeId]];
        // 商品状态
        $status > 0 && $filter[] = ['status', '=', $status];
        // 获取商品列表数据
        $data = $this->withoutField(['content'])
