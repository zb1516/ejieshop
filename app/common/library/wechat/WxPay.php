...
class WxPay extends WxBase
{
    ...
    private $modelClass = [
        ...
        OrderTypeEnum::USERCARD => \app\api\service\recharge\PayUserCardSuccess::class,
    ];
    ...
    public function unifiedorder($orderNo, $openid, $totalFee, int $orderType = OrderTypeEnum::ORDER): array
    {
        ...
        //直接支付成功 测试用 上线还原
        return $this->synchronization($params);
     ...
     */
    public function synchronization($data)
    {
        // 实例化订单模型
        ...
        $time = time();
        $prepay['prepay_id'] = rand(1111111111,2222222222222);
         // 生成 nonce_str 供前端使用
        $paySign = $this->makePaySign($data['nonce_str'], $prepay['prepay_id'], $time);
        return [
            'prepay_id' => $prepay['prepay_id'],
